...
volatile unsigned long movedtime = -50000;  // trigger PIR sensor motion movement detection a max of once every 5 secs
volatile bool moved = false;
volatile bool recordflag = true;
volatile bool lcdrecord = true;
// MQTT Settings
...
void IRAM_ATTR on_pir_motion_detected() {
  if (millis() - movedtime > 50000 && recordflag) {  // if moved, delay of 30s
    moved = true;
    recordflag = false;
    movedtime = millis();
  }
...
void loop() {
  ...
  switch (current_state)
  {
      case IDLE:
        ...
        {
          if (!enablePB) printLCD("Pls stand still\nCamera Capturing");
          else printLCD("Press button to\nrecord password");
          if (moved) {
            moved = false;
