// Modified script from library ESP32MQTTClient by Chen Yijun, found at https://github.com/cyijun/ESP32MQTTClient
// Modified to send a specified number of bytes rather than a string so that null bytes are included.
...
bool ESP32MQTTClient::publishFixedLength(char *topic, char *payload, int payload_length, int qos, bool retain)
{
    ...
    bool success = false;
    if (esp_mqtt_client_publish(_mqtt_client, topic, payload, payload_length, qos, retain) != -1)
    {
        success = true;
    }
