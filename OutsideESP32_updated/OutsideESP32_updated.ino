...
volatile unsigned long pressedtime = 0;     // to keep track of last PB pressed
volatile bool enablePB = false;             // to keep track whether PB can be pressed through object detection from esp32cam
volatile bool moved = false;                
volatile bool recordflag = true;            // will be disabled when motion detected, only enabled again when correct password or correct attempt
// MQTT Settings
...
char *pirmotion = "sensors/motion";                                  // Published when PIR sensor triggers (sensitive), triggering camera
char *startmicrophone = "actuators/microphone";
...
char *resetAttempts = "actuators/reset_attempts";
char *detectionCamera = "actuators/camera";
char *classificationdone = "actuators/PIR";
...
void IRAM_ATTR on_button_pressed() {
  ...
  if (millis() - pressedtime > RECORD_TIME * 1000 + 2000 && enablePB) {
    pressedtime = millis();
    pressed = true;
  }
}
...
void IRAM_ATTR on_pir_motion_detected() {
  if (recordflag) {  // only allow recording if reset or correct password 
    moved = true;
    recordflag = false;
  }
}
...
void loop() {
  ...
  switch (current_state)
  {
      case IDLE:
        ...
        {
          if (enablePB) printLCD("Press button to\nrecord password"); 
          else printLCD("Pls stand still\nCamera Capturing");
          if (moved) {
            moved = false;
            Serial.println("Motion detected");
            String msg_moved = "moved";
            mqttClient.publish(pirmotion, msg_moved);  // will trigger esp32cam
          }
        }
        // Pushbutton
        if (pressed) {
          ...
          mqttClient.publish(startmicrophone, "1");  // Send MQTT to another esp32 for recording
          printLCD("Recording");
          pressed = false;
        }
        delay(10);
...
void onConnectionEstablishedCallback(esp_mqtt_client_handle_t client) {
  if (mqttClient.isMyTurn(client))  // can be omitted if only one client
  {
    ...
    mqttClient.subscribe(classificationdone, [](const String &payload) {
      recordflag = true;
    });
    mqttClient.subscribe(detectionCamera, [](const String &payload) {
      enablePB = true;  // enable PushButton presses only if camera detects a person
    });
